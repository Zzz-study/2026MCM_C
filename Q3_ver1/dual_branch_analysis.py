...
def create_data_loaders(df_train, df_val, df_test, feature_cols, batch_size=64):
    ...
    y_judge_train = torch.FloatTensor(df_train['y_judge'].values).unsqueeze(1)
    y_fan_train = torch.FloatTensor(df_train['y_fan'].values).unsqueeze(1)
    
    X_val = torch.FloatTensor(df_val[feature_cols].values)
    y_judge_val = torch.FloatTensor(df_val['y_judge'].values).unsqueeze(1)
    y_fan_val = torch.FloatTensor(df_val['y_fan'].values).unsqueeze(1)
    
    X_test = torch.FloatTensor(df_test[feature_cols].values)
    y_judge_test = torch.FloatTensor(df_test['y_judge'].values).unsqueeze(1)
    y_fan_test = torch.FloatTensor(df_test['y_fan'].values).unsqueeze(1)
    ...
    return train_loader, val_loader, test_loader, (X_test, y_judge_test, y_fan_test)
...
class SHAPAnalyzer:
    ...
    def analyze_global_shap(self, X_test_tensor, feature_names, n_background=100):
        ...
        # 准备背景数据
        if len(X_test_tensor) > n_background:
            indices = np.random.choice(len(X_test_tensor), n_background, replace=False)
            background = X_test_tensor[indices].to(self.device)
        else:
            background = X_test_tensor.to(self.device)
        
        # 创建SHAP解释器
        print(f"✓ 创建DeepExplainer (背景样本数: {len(background)})")
        
        # 为两个分支分别创建解释器
        # 评委评分分支
        def judge_model_wrapper(x):
            judge_out, _ = self.model(x)
            return judge_out
        
        explainer_judge = shap.DeepExplainer(judge_model_wrapper, background)
        
        # 粉丝投票分支
        def fan_model_wrapper(x):
            _, fan_out = self.model(x)
            return fan_out
        
        explainer_fan = shap.DeepExplainer(fan_model_wrapper, background)
        
        # 计算SHAP值
        print("计算SHAP值...")
        test_samples = X_test_tensor[:200].to(self.device)  # 使用前200个样本
        
        shap_values_judge = explainer_judge.shap_values(test_samples)
        shap_values_fan = explainer_fan.shap_values(test_samples)
        
        print(f"✓ SHAP计算完成")
        
        # 可视化
        self._visualize_shap(shap_values_judge, shap_values_fan, 
                           test_samples.cpu().numpy(), feature_names)
    ...
    def _visualize_shap(self, shap_judge, shap_fan, test_data, feature_names):
        ...
        shap.summary_plot(shap_judge, test_data, feature_names=feature_names, show=False)
        ...
        # 粉丝投票SHAP摘要图
        plt.figure(figsize=(12, 8))
        shap.summary_plot(shap_fan, test_data, feature_names=feature_names, show=False)
    ...
    def _generate_feature_impact_report(self, shap_judge, shap_fan, feature_names):
        ...
        # 归一化重要性
        judge_importance_norm = judge_importance / judge_importance.sum()
        fan_importance_norm = fan_importance / fan_importance.sum()
        
        # 确定影响类型
        impact_types = []
        for j_mean, f_mean, j_imp, f_imp in zip(judge_mean_shap, fan_mean_shap, 
                                                 judge_importance, fan_importance):
...
def main():
    ...
    # 步骤7: 创建数据加载器
    train_loader, val_loader, test_loader, test_tensors = create_data_loaders(
