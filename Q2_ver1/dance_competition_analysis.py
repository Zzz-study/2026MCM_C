...
def random_forest_stratified_model(final_vote_df):
    ...
    # 检查预测结果的类别数
    unique_pred = np.unique(y_pred_clf)
    unique_true = np.unique(y_clf)
    print(f"  - 预测类别: {unique_pred.tolist()}")
    print(f"  - 真实类别: {unique_true.tolist()}")
    
    # 混淆矩阵（使用labels参数确保2x2矩阵）
    cm = confusion_matrix(y_clf, y_pred_clf, labels=[0, 1])
    ...
    if cm.shape == (2, 2):
        ...
        print(f"\n  警告: 混淆矩阵形状异常 {cm.shape}")
        print(f"  混淆矩阵:\n{cm}")
        print(f"  可能原因: 模型预测结果只包含单一类别")
