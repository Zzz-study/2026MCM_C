...
```bash
# 进入项目根目录
cd "D:\Mathematical modeling\MCM_C_Final"
...
# 提交更改
git commit -m "Initial commit: MCM Problem C Dance Competition Analysis"
```

#### 步骤3：创建GitHub仓库并推送

```bash
# 在GitHub上创建新仓库后，添加远程地址
git remote add origin https://github.com/YOUR_USERNAME/MCM_C_DanceCompetition.git
...
2. **添加本地仓库**:
   ...
   - 选择 `D:\Mathematical modeling\MCM_C_Final`
...
1. **打开项目**: 在VS Code中打开 `MCM_C_Final` 文件夹
...
1. **数据文件**: 大型CSV数据文件建议使用Git LFS或不上传，在README中提供数据来源说明

2. **敏感信息**: 确保没有上传API密钥、密码等敏感信息

3. **许可证**: 建议添加LICENSE文件（MIT、GPL等）

4. **文档**: 确保README.md完整描述项目功能和使用方法
